 ...
 *  Reply with a tile
 * --------------------------------------------------------------------------*/
int http_reply_tile( cnx_t *cnx, char *mtype, int x, int y, int z )
{
  char *data = NULL;
  int len = 0;

  fprintf(stderr, "http_reply_tile: %d/%d/%d\n", z, x, y );
...
int http_reply( cnx_t *cnx )
{
  ...
  case HTTP_GET:
    ...
    if ( !strncmp( k, "tiles/", 6) ) {
      if ( isdigit(k[6]) ) {
	n = sscanf( k, "tiles/%d/%d/%d.pbf", &z, &x, &y );
	if ( n == 3 ) {
	  return http_reply_tile( cnx, "application/x-protobuf", x, y, z);
	}
	else {
	  return http_reply_error( cnx, HTTP_STATUS_NOT_FOUND );
	}
